...
const transferAction: Action = {
  ...
  description: `This action/tool is used to transfer SOL, the native token of Solana,
   to another wallet address which is a solana public key. Address of SOL is So11111111111111111111111111111111111111112.
    When using this action, the user is aware that they are transferring SOL and not any other token.`,
  examples: [
    [
      {
        input: {
  ...
  handler: async (agent: SolanaAgentKit, input: Record<string, any>) => {
    ...
    const tx = await transfer(agent, recipient, input.amount);
